...
/// </summary>
public sealed class SinkChain<T, THead>
    where T    : unmanaged
    where THead : DispatchSink<T>
{
    private readonly THead      _head;
    private DispatchSink<T>     _tail;

    internal SinkChain(THead head)
    {
    ...
    /// <paramref name="next"/> is a <see cref="SpscQueueSink{T}"/> or <see cref="MpscQueueSink{T}"/>,
    /// wires its <c>Prev</c> pointer so recovery can drain back to the predecessor.
    /// </summary>
    public SinkChain<T, THead> To(DispatchSink<T> next)
    {
    ...
    /// to <see cref="DispatchSink{T}.Next"/>.
    /// </summary>
    ...
    /// registered via <paramref name="configure"/>. Fallback to <see cref="DispatchSink{T}.Next"/>
    /// only fires when all branches are unhealthy.
    /// </summary>
    public SinkChain<T, THead> Multi(Action<MultiBuilder<T>> configure)
    {
    ...
    /// <summary>
    /// Fixed-arity 2-branch broadcast using the CRTP <see cref="Multi2Sink{T,TC1,TC2}"/> variant.
    ...
    /// </summary>
    public SinkChain<T, THead> Multi<TC1, TC2>(TC1 c1, TC2 c2)
        where TC1 : DispatchSink<T>
        where TC2 : DispatchSink<T>
    {
        var multi  = new Multi2Sink<T, TC1, TC2>(c1, c2);
        _tail.Next = multi;
        _tail      = multi;
        return this;
    }
    ...
    internal void AppendFilter(FilterSink<T> filter, DispatchSink<T> downstream)
    {
        _tail.Next = filter;
    ...
    private void WirePrev(DispatchSink<T> next)
    {
        switch (next)
        {
            case SpscQueueSink<T> sq: sq.Prev = _tail; break;
            case MpscQueueSink<T> mq: mq.Prev = _tail; break;
        }
    }
}
